...
/**
 * Seed the adtruck-e2e Supabase project with fixture users + campaign
 * used by the mobile E2E (Maestro Cloud) pipeline.
 *
 * Runs via: pnpm seed:e2e  — requires .env.e2e.local with:
 *   SUPABASE_URL_E2E
...
function required(key: string): string {
  ...
  if (!v) {
    ...
    console.error('Add it to .env.e2e.local or export it before running.');
    process.exit(1);
  }
  return v;
}

async function ensureUser(admin: SupabaseClient, u: FixtureUser): Promise<string> {
...
async function ensureCampaign(
  ...
  ids: { adminId: string; driverId: string; clientId: string },
): Promise<void> {
  ...
  const { error } = await admin.from('campaigns').upsert(
    {
      ...
      driver_profile_id: ids.driverId,
...
async function run(): Promise<void> {
  ...
  const { error: driverErr } = await admin
    .from('drivers')
    .upsert({ profile_id: driverId, city: 'Chicago' }, { onConflict: 'profile_id' });
  if (driverErr)
    throw driverErr;
  console.log(`  • upserted driver for ${driverId}`);

  console.log('Seed campaign:');
  await ensureCampaign(admin, { adminId, driverId, clientId });
